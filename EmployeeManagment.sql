...
CREATE PROCEDURE spSTORED2
 ...
 @result INT OUTPUT
  ...
  CREATE PROCEDURE spSTORED3
  AS
  BEGIN TRY 
    SELECT * FROM Project WHERE project_name = 'Project 1'
  END TRY
  BEGIN CATCH
...
SELECT e.emp_id, e.fname, d.department_name
FROM Employee e JOIN Department d 
ON e.emp_id = d.department_id

SELECT * FROM EmployeeView
SELECT fname FROM EmployeeView
...
SELECT * FROM EmployeeSummaryView

-- if we want to delete view
DROP VIEW EmployeeSummaryView

----------------------------------------------	SUB QUERY -------------------------------------------------------------------------
--SINGLE ROW SUBQUERY (use ' = ' to get 0 or more row)
/* Create the database */
CREATE DATABASE EmployeeManagement
...
SELECT emp_id,fname FROM employee
 WHERE emp_id = (
   SELECT emp_id FROM employee WHERE lname = 'Pandey'
)


-- single row with join
SELECT e.emp_id,e.fname,e.lname FROM employee e JOIN Department d ON  e.emp_id = d.department_id
 WHERE department_id = (
...
--USING IN 
SELECT e.emp_id,e.fname,e.lname FROM employee e JOIN payroll p ON e.emp_id = p.payroll_id
WHERE basic_pay IN (
SELECT basic_pay FROM payroll WHERE payroll_id > 7
 )

-- USING NOT IN (work as except theses print all)
SELECT e.emp_id,e.fname,e.lname FROM employee e JOIN payroll p ON e.emp_id = p.payroll_id
WHERE basic_pay NOT IN (
SELECT basic_pay FROM payroll WHERE payroll_id > 7
 )

-- USING = ANY (work similar as IN)
SELECT e.emp_id,e.fname,e.lname FROM employee e JOIN payroll p ON e.emp_id = p.payroll_id
WHERE basic_pay = ANY (
SELECT basic_pay FROM payroll WHERE payroll_id > 7
 )

 --USING  > ANY and < ANY (it require single vlaue op of sub query to compare)
 SELECT e.emp_id,e.fname,e.lname FROM employee e JOIN payroll p ON e.emp_id = p.payroll_id
WHERE basic_pay > ANY (
SELECT AVG(basic_pay) FROM payroll
 )

  SELECT e.emp_id,e.fname,e.lname FROM employee e JOIN payroll p ON e.emp_id = p.payroll_id
WHERE basic_pay < ANY (
SELECT AVG(basic_pay) FROM payroll WHERE payroll_id > 7
 )

 --  WITHOUT ANY IT WILL ALSO WORK
 SELECT emp_id, fname FROM employee WHERE emp_id > (
   SELECT emp_id FROM employee WHERE lname = 'Mishra'
   )

 ----- we can copy one table file to another table with insert
  /* CREATE NEW TABLE */
  
/* Create table 5: Project1 */
